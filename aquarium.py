...
if df.empty:
    ...
    if not ausgewaehlte_parameter:
        ...
        # Nur Daten von Aquarien anzeigen, die auch aktuell in der Liste existieren
        df_aktiv = df[df["Aquarium"].isin(aquarien_optionen)]
        ...
        if modus == "Einzelansicht":
            ...
            if df_aktiv.empty:
                st.warning("Keine Daten für die aktiven Aquarien vorhanden.")
            else:
                df_melted = df_aktiv.melt(id_vars=["Datum", "Aquarium"], value_vars=ausgewaehlte_parameter, var_name="Parameter", value_name="Wert")
    ...
    df_anzeige = df[df["Aquarium"].isin(aquarien_optionen)].copy()
    ...
    if st.button("💾 Änderungen in Tabelle speichern"):
        ...
        # Behalte im Haupt-DF auch die Daten eventuell gelöschter Aquarien im Hintergrund,
        # überschreibe aber die editierten aktiven Daten
        df_andere = df[~df["Aquarium"].isin(aquarien_optionen)]
        df_final = pd.concat([df_andere, edited_df], ignore_index=True).sort_values(by="Datum")
    ...
    df_excel = df[df["Aquarium"].isin(aquarien_optionen)].copy()
    ...
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        for aq in aquarien_optionen:
            ...
            if not df_aq.empty:
                df_aq.to_excel(writer, sheet_name=aq[:31], index=False)
        if not df_excel.empty:
            df_excel.to_excel(writer, sheet_name="Alle Daten", index=False)
        else:
            # Fallback, falls gar keine Daten da sind, damit ExcelWriter nicht abstürzt
            pd.DataFrame(columns=["Status"]).to_excel(writer, sheet_name="Keine Daten", index=False)
...
# --- VERWALTUNGSBEREICH JETZT IN 3 SPALTEN ---
st.markdown("---")
...
with col_del:
    ...
    zu_loeschen = st.selectbox("Welches Becken möchtest du entfernen?", aquarien_optionen, key="del_select")
    
    # Sicherheitsabfrage via Checkbox, damit man nicht aus Versehen klickt
    sicherheit = st.checkbox("Ja, ich möchte dieses Aquarium aus der Auswahlliste entfernen.")
    
    if st.button("❌ Aquarium entfernen"):
        if sicherheit:
            if len(aquarien_optionen) > 1:
                aquarien_optionen.remove(zu_loeschen)
                speichere_aquarien(aquarien_optionen)
                st.success(f"'{zu_loeschen}' wurde aus der Liste entfernt!")
                st.rerun()
            else:
                st.error("Es muss mindestens ein Aquarium in der Liste bleiben!")
        else:
            st.warning("Bitte setze erst das Häkchen zur Bestätigung.")